...
opBegin(map::IndexAssign)
    ...
    opDoc("Assigns an element to the given index of the map. Any already existing element will be overwritten.")
...
opBegin(map::Get : MethodCall)
    ...
    opDoc("Returns the map element at the given *index*. If the element does not exist, *default* is returned if given.")

    opValidate() {
        auto mtype = ast::checkedCast<type::Map>(op1()->type());
        type_list args = { mtype->keyType(), std::make_shared<type::OptionalArgument>(mtype->valueType()) };
...
opBegin(map::Timeout : MethodCall)
    opOp1(std::make_shared<type::Map>())
    ...
    opDoc("Activates automatic expiration of items for the map.")
